...
(function(){
	...
	.factory("associadoUtil",function($uibModal){
           
		var _openItem = function(item, callback){
			
			$uibModal.open({
				animation: true,
				templateUrl:"app/associado/html/detalhe.html",
				size:'lg',
				controllerAs:"vm",
				controller: function(stService, stUtil, $modalInstance, $route){
					...
					vm.salvarItem = function(){
						...
						stService.executePost("associado/add/", _item).success(function(data){
					...
					vm.deletarItem = function(){
						...
						stService.executePost("associado/delete/", [vm.item.id]).success(function(){

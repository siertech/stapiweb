...
(function(){
	...
	.factory("imovelUtil",function($uibModal){
           
		var _openItem = function(item, callback){
			
			$uibModal.open({
				animation: true,
				templateUrl:"app/imovel/html/detalhe.html",
				size:'lg',
				controllerAs:"vm",
				controller: function(stService, stUtil, $modalInstance, $route){
					...
					vm.salvarItem = function(){
						...
						stService.executePost("imovel/add/", _item).success(function(data){
					...
					vm.deletarItem = function(){
						...
						stService.executePost("imovel/delete/", [vm.item.id]).success(function(){

...
(function(){
	...
	.factory("licitacaoUtil",function($uibModal){
           
		var _openItem = function(item, callback){
			
			$uibModal.open({
				animation: true,
				templateUrl:"app/licitacao/html/detalhe.html",
				size:'lg',
				controllerAs:"vm",
				controller: function(stService, stUtil, $modalInstance, $route){
					...
					vm.salvarItem = function(){
						...
						stService.executePost("licitacao/add/", _item).success(function(data){
					...
					vm.deletarItem = function(){
						...
						stService.executePost("licitacao/delete/", [vm.item.id]).success(function(){

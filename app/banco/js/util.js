...
(function(){
	...
	.factory("bancoUtil",function($uibModal){
           
		var _openItem = function(item, callback){
			
			$uibModal.open({
				animation: true,
				templateUrl:"app/banco/html/detalhe.html",
				size:'lg',
				controllerAs:"vm",
				controller: function(stService, stUtil, $modalInstance, $route){
					...
					vm.salvarItem = function(){
						...
						stService.executePost("banco/add/", _item).success(function(data){
					...
					vm.deletarItem = function(){
						...
						stService.executePost("banco/delete/", [vm.item.id]).success(function(){
